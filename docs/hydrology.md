...
The render geometry pass builds fractional points directly from the canonical river cells. It may add small bends and jitter for visual quality, but it no longer traces a separate integer channel that can diverge from the exported cells. This separates stable drainage accounting from cartographic river shape: `flowDirections` drives accumulation and basin logic, `RiverSegment.Cells` stores the canonical visible channel path, and `RiverSegment.Polyline` stores the smoothed render geometry. The render layer may smooth a river, but it must not introduce a new crossing or confluence outside the canonical channel.

During channel tracing, a straight-run repair pass looks for repeated direction vectors of six or more cells in `RiverSegment.Cells`. When it finds one, it attempts a local least-cost reroute between neighboring anchors in a 3..8 cell corridor, forbidding continuation of the same diagonal vector beyond the local limit and softly preferring valley/basin cells while allowing only a small breach. If the local search cannot find a better path, a short adjacent-cell detour can split the run while preserving the segment endpoints. The same repair runs again after zig-zag cleanup so smoothing cannot recreate long straight exported cell paths. A canonical crossing repair then turns any new opposing diagonal river edges into confluences before the final polyline crossing pass runs.
...
- `DeltaFrequency`
- `ChannelCurvatureStrength` (default `1`, `0` keeps canonical paths close to the hydrology graph, values above `1` split straight runs more aggressively)
- `MeanderStrength` (default `0.65`, render-polyline meander amplitude)
