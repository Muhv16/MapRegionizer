...
The `Quality` block reports straight-run count and maximum run length, short-river count using a scale-dependent 5..8 cell limit, detached-river count, confluence count, mean tributaries per major river, endorheic river count, maximum alternating zig-zag run, mean curvature, sharp turns, and backtrack-like turns. `Summary.EndorheicRiverCount` is the count of exported rivers whose `Kind` is `Endorheic`. Separate summary counters report lake inflow rivers, closed-lake rivers, open-lake rivers, and dry-basin rivers so closed lakes and dry basins no longer have to be inferred from one overloaded count. `Summary.MajorRiverCount` uses the exported `IsMajor` classification so tiny creeks and steep low-discharge streams do not inflate the major-river total.
