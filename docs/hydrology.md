...
Flow routing still builds a D8-style `flowDirections` raster for hydrology, but visible rivers are no longer rendered directly from a single greedy chain. The hydrology raster remains the source of accumulation, basins, lake targets, and terminal drainage. Before final accumulation, the flow graph runs a canonical straight-run regularization pass over potentially visible drainage: repeated direction vectors are locally rerouted between the same upstream/downstream anchors in a bounded land corridor, with cycle checks and terminal preservation. Visible river cells are then projected into a separate integer `RiverTopologyGraph`; after canonical river segments are extracted from that graph, each segment receives a smoothed polyline derived from its canonical cell path for rendering.
...
In low-slope plains, highlands, and foothills, the hydrology routing cost also applies a deterministic multi-scale lateral field and a small grid-line penalty. Lake and ocean target attraction remains, but is softened in flat terrain so rivers do not always snap into the nearest water cell along long straight runs. Small dry depressions can be crossed by a limited logical breach when the uphill cost is low, especially near passes or basin outlets. Lakes without outlets are terminal drainage targets unless a shallow non-inland-sea lake receives many significant inflows; in that case a forced outlet is placed on shoreline far from the inflow cluster. `LakeOutletInflowForceMultiplier` scales the inflow-count threshold for this forced outlet.

Forced long-river and guaranteed-tributary selection is shape-aware. When several upstream branches have similar depth or discharge, the extractor avoids extending the same D8 vector for too long and mildly prefers soft turns over diagonal or axis-locked runs. This keeps long mainstems and mountain tributaries from inheriting straight artifacts from a locally plausible but visually implausible flow graph.

The visible canonical-cell pass can also retrace a segment through a bounded corridor before export. This pass updates `RiverSegment.Cells`, so quality diagnostics and downstream renderers see the corrected integer path rather than a cosmetic-only bend. Exported visible paths are required to be self-simple: a river segment may not repeat a cell or cross itself in either its canonical cells or render polyline. If channel tracing creates a loop, the loop is removed or rerouted locally; if render bend points create a self-crossing, the polyline is rebuilt with weaker bends and then without bend points as a final fallback. Inter-river crossing repair remains separate and does not replace this per-river invariant. The render geometry pass then builds fractional points directly from those canonical river cells. It may add small bends and jitter for visual quality, but it must not introduce a new crossing or confluence outside the canonical topology. This separates stable drainage accounting from cartographic shape: `flowDirections` drives accumulation and basin logic, `RiverSegment.Cells` stores the visible canonical integer path, and `RiverSegment.Polyline` stores the smoothed render geometry.
