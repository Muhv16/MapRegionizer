...
internal sealed class HydrologyGenerator
{
        ...
        HydrologyGenerationOptions options)
    {
        ...
        MarkForcedLongRiverCells(forcedLongPaths, riverCells, lakeIds);
        ...
        if (ResolveCrossingRiverEdges(mask, elevation, hydroSurface, flowDirections, accumulation, basinIds, riverCells, lakeIds))
        {
            BreakCycles(flowDirections, width, height);
            ResolveInvalidDryTerminals(mask, elevation, waterBodyTopology, generatedLakes, hydroSurface, lakeIds, flowDirections, options);
            BreakCycles(flowDirections, width, height);
            ...
            (basinIds, basins) = BuildBasins(mask, elevation, waterBodyTopology, waterSurfaces, flowDirections, accumulation, lakeIds);
            endorheicPolicies = BuildEndorheicRiverPolicies(basins, elevation);
            validEndorheicBasins = BuildValidEndorheicBasinSet(basins, endorheicPolicies);
            allowedRiverBasins = BuildAllowedRiverBasinSet(basins, validEndorheicBasins);
            forcedLongPaths = BuildForcedLongRiverPaths(mask, waterBodyTopology, flowDirections, accumulation, basinIds, basins, allowedRiverBasins, lakeIds, riverCells, options);
            MarkForcedLongRiverCells(forcedLongPaths, riverCells, lakeIds);
        }
    ...
    private static bool ResolveCrossingRiverEdges(
        MapMask mask,
        ElevationMap elevation,
        double[] hydro,
        int[] flowDirections,
    ...
    private static bool TryGetVisibleEdgeBetween(int first, int second, int[] flowDirections, byte[] riverCells, int width, int height, out RiverEdge edge)
    {
    ...
    private static bool TryChooseCrossingConfluenceTarget(
        RiverEdge weak,
        RiverEdge strong,
        MapMask mask,
        ElevationMap elevation,
    ...
    private static double CrossingConfluenceCost(
        RiverEdge weak,
        RiverEdge strong,
        int target,
        ElevationMap elevation,
    ...
    private static bool WouldCreateCycle(int from, int target, int[] flowDirections, int width, int height)
    {
        var current = target;
        var guard = 0;
    ...
    private static void MarkForcedLongRiverCells(IReadOnlyDictionary<int, IReadOnlyList<int>> forcedLongPaths, byte[] riverCells, int[] lakeIds)
    {
    ...
    private readonly record struct RiverEdge(int From, int To);
